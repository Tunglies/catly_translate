...
def __random_useragent():
    if not hasattr(__random_useragent, "__instance"):
        useragent = UserAgent()
        setattr(__random_useragent, "__instance", useragent)
    return getattr(__random_useragent, "__instance").random.__str__()
...
def __post_translate(text, from_lang="en", to_lang="zh"):
    ...
    headers = {
        ...
        'User-Agent': __random_useragent(), }
    data = httpx.post(url=url, headers=headers).json()
    return data
